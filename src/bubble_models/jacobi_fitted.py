...
@nb.njit(cache=True, fastmath=True, inline='always')
def _get_omega_opt(rmax, params):
    a, b, c = params
    return a * np.exp(c * rmax) + b             
...
@register_ode("jacobi_fitted")
@nb.njit(cache=True, fastmath=True)
def _ode_function(t, x, up, cp, d, num_bubbles, atol, rtol, max_iter, metrics_buffer, jacobi_params):
    """
    ...
    rmax = np.max(x1)
    omega_opt = _get_omega_opt(rmax, jacobi_params)
    dx_sol, iter, _, rnorm, step_norm = _jacobi_iter(rD, x1**2, cp, rhs, omega_opt, atol, rtol, max_iter)
    dx[num_bubbles:] = dx_sol

    metrics_buffer[0] = t
    metrics_buffer[1] = np.max(x1)
    metrics_buffer[2] = omega_opt              # This method does not use a relaxation factor
